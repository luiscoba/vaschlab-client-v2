...
export class StudentsListComponent implements AfterViewInit {
  ...
  studentSchoolIdFormControl = new FormControl('', {
    updateOn: 'blur',
  ...
  masterToggle(): any {
    if (this.isAllSelected()) {
      ...
      this.studentsHttpClient
        ...
        .pipe(
          map((studentsList) => {
            // the function to apply to the StudentDetailList observable
            this.isProcessing = false;
            return studentsList.students;
          })
        )
        .subscribe((students) => {
          students.forEach((row) => this.selectedStudents.add(row));
        });
    }
  }
   ...
   */
  openTextFilterDialog(parentElement: any, column: string, title: string): any {
    ...
    const modalRef = this.modalService.open(TableFilterTextComponent, {
      size: 'sm',
    });
    ...
    modalRef.result.then((result: TableFilter) => {
      if (result.value.length === 0) {
        ...
        this.tableFilters.set(result.field, {
          field: `${result.field}`,
  ...
  onEditClick(studentId: number): void {
    ...
    const student: Student = this.students.find(
  ...
  isUpdateDisabled(): boolean {
    ...
    if (
      this.studentSchoolIdFormControl.invalid ||
      this.firstNameFormControl.invalid ||
      this.lastNameFormControl.invalid ||
      this.studentEmailFormControl.invalid
    ) {
      return true;
    }
  ...
  onUpdateClick(studentId: number): void {
    ...
    this.isProcessing = true;
    this.studentsHttpClient.updateStudent(student).subscribe((rsp) => {
      this.isProcessing = false;
      if (rsp.success) {
  ...
  onAddClick(): void {
    ...
    this.isProcessing = true;
    this.studentsHttpClient.addStudent(student).subscribe((rsp) => {
      this.isProcessing = false;
      if (rsp.success) {
  ...
  onDeleteSelectedClick(): void {
    ...
    if (this.selectedStudents.size === 0) {
      const modalRefInfo = this.modalService.open(NotificationComponent, {
        size: 'sm',
        centered: true,
      });
    ...
    modalRef.result.then((confirmedDelete: boolean) => {
      if (confirmedDelete) {
        ...
        this.studentsHttpClient.deleteStudents(studentIds).subscribe((rsp) => {
          this.isProcessing = false;
          if (rsp.success) {
            // Reload the students passing in a post load function that sets editStudentId = -1
  ...
  onDeleteClick(studentId: number): void {
    ...
    modalRef.result.then((confirmedDelete: boolean) => {
      if (confirmedDelete) {
        this.isProcessing = true;
        this.studentsHttpClient.deleteStudent(studentId).subscribe((rsp) => {
          this.isProcessing = false;
          if (rsp.success) {
            // Reload the students passing in a post load function that sets editStudentId = -1
            // so the row will exit edit mode
  ...
  reloadStudents(postLoadFunction: () => void): void {
    ...
    this.studentsHttpClient
      ...
      .pipe(
        map((studentsList) => {
          // the function to apply to the StudentsList observable
  ...
  studentSchoolIdValidator(): AsyncValidatorFn {
    return (control: AbstractControl): Observable<ValidationErrors | null> => {
      ...
      this.isProcessing = true;
      return this.studentsHttpClient
        .validateStudentSchoolId(this.editStudentId, control.value)
        .pipe(
          switchMap((rsp) => {
            // switches the Response observable to a ValidationErrors observable
            this.isProcessing = false;
            if (rsp.success) {
              return of(null);
            } else {
